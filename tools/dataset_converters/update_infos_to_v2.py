...
def update_donaset_infos(pkl_path, out_dir):
    print(f'{pkl_path} will be modified.')
    if out_dir in pkl_path:
        print(f'Warning, you may overwriting '
              f'the original data {pkl_path}.')
    ...
    for ori_info_dict in mmengine.track_iter_progress(data_list):
        ...
        if anns is not None:
            ...
            for instance_id in range(num_instances):
                ...
                empty_instance['truncated'] = None #TEST
                # empty_instance['occluded'] = anns['occluded'][instance_id].tolist()
                empty_instance['occluded'] = None # TEST
                
                # empty_instance['alpha'] = anns['alpha'][instance_id].tolist()
                empty_instance['alpha'] = None # TEST
    ...
    mmengine.dump(converted_data_info, out_path, 'pkl')
def update_kittilidar_infos(pkl_path, out_dir):
    print(f'{pkl_path} will be modified.')
    if out_dir in pkl_path:
        print(f'Warning, you may overwriting '
              f'the original data {pkl_path}.')
...
def update_pkl_infos(dataset, out_dir, pkl_path):
    ...
    elif dataset.lower() == 'donaset':
        update_donaset_infos(pkl_path=pkl_path, out_dir=out_dir)
    elif dataset.lower() == 'kittilidar':
        update_kittilidar_infos(pkl_path=pkl_path, out_dir=out_dir)
